-- AUTO WALLHOP + DOUBLE JUMP (FLICK 100% FÍSICO - SEM ROTAÇÃO)
...
local function performVideoFlick()
    ...
    -- direção baseada na câmera (sem rotacionar nada)
    ...
    -- forças do flick físico
    local lateralForce = 38
    local forwardForce = 12

    local side = (math.random() < 0.5) and -1 or 1

    local velocityBoost =
        (flatRight * lateralForce * side) +
        (flatLook * forwardForce)

    -- aplica sem alterar rotação/câmera
    hrp.Velocity = Vector3.new(
        hrp.Velocity.X + velocityBoost.X,
        hrp.Velocity.Y,
        hrp.Velocity.Z + velocityBoost.Z
    )
...
print("WallHop Loaded (100% físico, sem rotação)")
