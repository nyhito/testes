...
-- CORRIGIDO:
-- agora só aceita a linha se houver parede ACIMA e ABAIXO do ponto,
-- além de existir uma quebra horizontal real perto da linha.
local function hasValidHorizontalEdge(rayResult, params)
    ...
    local probeDistance = 0.24

    local function hitsSame(yOffset)
        local origin = hitPos + Vector3.new(0, yOffset, 0) + surfaceOffset
        local probe = workspace:Raycast(origin, -normal * probeDistance, params)
        return probe and probe.Instance == inst
    end

    local aboveNear = hitsSame(0.35)
    local aboveFar  = hitsSame(0.80)
    local belowNear = hitsSame(-0.35)
    local belowFar  = hitsSame(-0.80)

    local hasWallAbove = aboveNear or aboveFar
    local hasWallBelow = belowNear or belowFar

    -- se faltar parede em qualquer lado da linha, invalida
    if not hasWallAbove or not hasWallBelow then
        return false
    end

    -- precisa existir uma quebra real perto da linha, senão vira parede comum
    local edgeBreak =
        (aboveNear ~= belowNear) or
        (aboveFar ~= belowFar) or
        (aboveNear ~= aboveFar) or
        (belowNear ~= belowFar)

    if not edgeBreak then
        return false
    end
...
print("Made by netzwwwiiiiii | Humanoid Wallhop - Loaded Successfully ✅")
